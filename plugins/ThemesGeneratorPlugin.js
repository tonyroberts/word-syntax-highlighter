...
class ThemesGeneratorPlugin {
    ...
    generateExtraOptions() {
        const { outputDir, themesLoader, filesLoader, defaultStyleName, useStaticThemeName } = this.options;
        this.generateThemes();
        ...
        const themeLoaders = themeFileNames.map((fileName, index) => {
            if (filesLoader && filesLoader.test && fileName.match(filesLoader.test)) {
                return {
                    test: filesLoader.test,
